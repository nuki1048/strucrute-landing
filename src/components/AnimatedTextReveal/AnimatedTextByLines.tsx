...
// BrightTextRichLines.tsx (LCP optimized version)
...
export function BrightTextRichLines({
  ...
  // LCP Optimization: Show text immediately, then animate
  React.useEffect(() => {
    // Show text immediately for LCP
    setIsVisible(true);
  ...
  // autoplay path
  React.useEffect(() => {
    if (externalProgress || !autoplay || !isVisible) return;
  ...
  // LCP Optimization: Always render text, but with different opacity
  return (
    <chakra.span
      className={hideUntilStart && !started ? "bt-no-prepaint" : undefined}
      style={{
        // Show text immediately for LCP, but with reduced opacity
        visibility: isVisible ? "visible" : "hidden",
        opacity: isVisible ? (hideUntilStart && !started ? 0.3 : 1) : 0,
      }}
      {...textProps}
      // @ts-ignore
      sx={{
...
function AnimatedLine({
  ...
  // LCP Optimization: Show text immediately with fallback color
  const fallbackColor = isVisible && !started ? bright : undefined;
